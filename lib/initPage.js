...
(function(window,undefined){
	...
	function initPage(id,option){
		...
		if(isJump){
			...
			input.style.border='1px solid #999999';
			...
			button.style.border='1px solid #08abff';
	...
	function createPageList(curPage,totalPage,showCount){
		...
		for(i=0;i<lis.length;i++){
			...
			tempLi.firstElementChild.style.color = '#0ae';
			if(tempLi.className == 'active'){
				tempLi.childNodes[0].style.backgroundColor = '#f8f8f8';
				tempLi.childNodes[0].style.color='#000';
			}
	...
	function createLink(str){
		if(str=="上一页"){
			return "<a href='#' style='font-size:22px;' class='icon-caret-right'></a>";
		}else if(str=="下一页"){
			return "<a href='#' style='font-size:22px;' class='icon-caret-left'></a>";
		}else{
			return "<a href='#'>" + str + "</a>";
		}
		console.log(str);
